...
class Serpent(object):
	...
	def get_coord (self) :
		'''Récupérer les coordonnées des divers morceaux du Serpent'''
		return self.coords
	...
	#Récupérer la direction du Serpent.	
	def get_direction(self) :
		'''Retourne la direction sous forme (x,y)'''
	...
	def calculCoord(self,direction) :
		...
		for i in range(0,2) :
			...
			while nvCoord[i]<0 or nvCoord[i] >= Plateau.TAILLE_MAX :
				if nvCoord[i]<0 : nvCoord[i]+=Plateau.TAILLE_MAX
				if nvCoord[i]>=Plateau.TAILLE_MAX : nvCoord[i]-=Plateau.TAILLE_MAX
...
class Plateau(object) :
	...
	def get_pommes(self) : 
		"""Retourne les coordonnées des pommes"""
		return self.pommes
	def get_poires(self) : 
		"""Retourne les coordonnées des poires"""
		return self.poires
	def get_murs(self)   : 
		"""Retourne les coordonnées des murs"""
		return self.murs
	...
	def aPerdu(self) :
		...
		for e in self.s.get_coord() :
			...
			if e in self.murs :
				...
				return True
			if self.testDoublon(self.s.get_coord()) : 
	...
	def toString(self) :
		...
		for a in range (0,Plateau.TAILLE_MAX) :
			for b in range (0,Plateau.TAILLE_MAX) :
				...
				for element in self.s.get_coord() :
					if element[0] == a and element[1] == b:
						bSerp=True
						if element == self.s.get_tete() :
							bTete=True
...
if __name__ == '__main__':
	...
	while not ( m.aGagner() or m.aPerdu() ) :
		...
		direction = ()
		if   (car == 'N') : direction = Direction.NORD
		elif (car == 'E') : direction = Direction.EST
		elif (car == 'S') : direction = Direction.SUD
		elif (car == 'O') : direction = Direction.OUEST		
		m.s.set_direction(direction)
		m.s.avancer()
		m.aManger()

	if m.aGagner() : print "Bravo !"
	else : print "Désolé !"
