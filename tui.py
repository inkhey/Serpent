...
def message(s) :
    ...
    ecran.addstr(Plateau.TAILLE_MAX/2,Plateau.TAILLE_MAX+5,s) # message au centre de la ligne
...
while not ( m.aGagner() or m.aPerdu() ) :
	...
	ecran.addstr(Plateau.TAILLE_MAX+1,0,(Plateau.TAILLE_MAX+2)*"*",curses.color_pair(4))
	...
	c=ecran.getch()
	if   (c == curses.KEY_LEFT)   : direction = Direction.OUEST 
	elif (c == curses.KEY_RIGHT)  : direction = Direction.EST
	elif (c == curses.KEY_UP)     : direction = Direction.NORD
	elif (c == curses.KEY_DOWN)   : direction = Direction.SUD	
	else : direction = m.s.get_direction()
